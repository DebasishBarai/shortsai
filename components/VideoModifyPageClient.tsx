...
export function VideoModifyPageClient({ videoId }: VideoPageClientProps) {
  ...
  const getVideo = useCallback(async (videoId: string) => {
    try {
      ...
      if (!videoData.url || !videoData.completed || !videoData.renderId) {
  ...
  const renderVideoContent = () => {
    ...
    return (
      <VideoPlayer
        ...
        height={675}
        frames={video.frames || []}
        withImages={video.imagesUrl && video.imagesUrl.length > 0 ? true : false}
